...
OPERATIONS = [
    ...
    ("POST", "/public/otp/send", "Public", "OTP 인증 시작(PDF MVP 경로)", "VerificationStartRequest", None),
    ("POST", "/public/verifications/confirm", "Public", "OTP 인증 확인", "VerificationConfirmRequest", None),
    ("POST", "/public/otp/confirm", "Public", "OTP 인증 확인(PDF MVP 경로)", "VerificationConfirmRequest", None),
    ...
    ("GET", "/public/upsell-hint", "Public", "다음 리워드 업셀 정보(PDF MVP 경로)", None, "portal"),
    ...
    ("POST", "/public/rewards/{grantId}/choose", "Rewards", "리워드 혜택 선택(PDF MVP 경로)", "RewardChooseRequest", "portal"),
    ...
    ("POST", "/admin/login", "Admin Auth", "관리자 로그인(PDF MVP 경로)", "LoginRequest", None),
    ...
    ("GET", "/admin/passes/active", "Admin WiFi", "실시간 이용권 목록(PDF MVP 경로)", None, "admin"),
    ...
    ("POST", "/admin/passes/{passId}/extend", "Admin WiFi", "이용권 수동 연장(PDF MVP 경로)", "PassExtendRequest", "admin"),
    ("POST", "/admin/passes/{passId}/expire", "Admin WiFi", "이용권 즉시 종료(PDF MVP 경로)", None, "admin"),
...
schemas = {
    ...
    "PassExtendRequest": object_schema(
        {**store_id, "minutes": {"type": "integer", "minimum": 1}},
        ["storeId", "minutes"],
    ),
